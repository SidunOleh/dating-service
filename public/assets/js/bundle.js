...
function initializeBattle($battle) {
    ...
    $battle.on("click", ".repeat.active", function () {
        ...
        setTimeout(
            () => activateRepeatButton($this),
            DS.ads.settings.repeat_time * 1e3 + 750
        );
    });
...
const withdraw = { payment_id: null, address_to: null, amount: null };
...
$(".crypto-address .next").on("click", async () => {
    ...
    $.post("/payments/withdraw/send-code", { ...withdraw, gateway: "crypto" })
        .done(() => {
