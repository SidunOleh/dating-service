...
codeInputs.forEach((codeInput, i) => {
    ...
    codeInput.addEventListener("input", (e) => {
        ...
        const lastInputBox = i === codeInputs.length - 1;
...
$("#delete-popup #confirm-delete:not(.load)").click(function () {
    $(this).addClass("load");
    $.ajax({ type: "DELETE", url: "/my-profile" })
        .then(() => {
...
function initializeBattle($battle) {
    ...
    function renderNextPair() {
        ...
        const html = (creator) => {
            return `
>>>>>>> 8d9c1355acc2d7893f63638522f90a9a45958b90
...
<<<<<<< HEAD
            </div>`};$battle.find(".photo:first").replaceWith(html(nextPair[0]));$battle.find(".photo:last").replaceWith(html(nextPair[1]))}function vote(id){$.post(`/roulette/vote/${id}`)}const $circle=$battle.find(".progress-ring__circle");const radius=$circle[0].r.baseVal.value;const circumference=2*Math.PI*radius;$circle.css({strokeDasharray:circumference,strokeDashoffset:circumference});let hasSelected=false;$battle.find(".start_btn").click(function(){const $this=$(this);$this.closest(".startBattle").addClass("hidden");startProgressAnimation($circle,DS.ads.settings.repeat_time*1e3);setTimeout(()=>activateRepeatButton($this),DS.ads.settings.repeat_time*1e3)});$battle.on("click",".photo",function(){const $selectedPhoto=$(this);if($selectedPhoto.hasClass("selected")){window.open(`/profile/${$selectedPhoto.data("id")}`,"_blank")}if(!hasSelected){const $blurredPhoto=$battle.find(".photo").not($selectedPhoto);selectPhoto($selectedPhoto,$blurredPhoto);hasSelected=true;vote($selectedPhoto.data("id"))}});$battle.on("click",".repeat.active",function(){setTimeout(()=>{renderNextPair();hasSelected=false;getNextPair()},500);const $this=$(this);const $loader=$battle.find(".loader");$this.removeClass("active");$loader.addClass("open");setTimeout(()=>{$loader.removeClass("open")},750);restartProgressAnimation($circle,DS.ads.settings.repeat_time*1e3);setTimeout(()=>activateRepeatButton($this),DS.ads.settings.repeat_time*1e3+750)});function startProgressAnimation($circle,duration){setTimeout(()=>{$circle.css({transition:`stroke-dashoffset ${duration/1e3}s linear`,strokeDashoffset:"0"})},0)}function restartProgressAnimation($circle,duration){$circle.css({transition:"none",strokeDashoffset:circumference});setTimeout(()=>{startProgressAnimation($circle,duration)},750)}function activateRepeatButton($button){$button.closest(".battle").find(".repeat").addClass("active")}function selectPhoto($selectedPhoto,$blurredPhoto){$selectedPhoto.addClass("selected").removeClass("blurred");$blurredPhoto.addClass("blurred").removeClass("selected")}}$(".battle").each(function(){initializeBattle($(this))});$("#change-email-form").submit(async function(e){e.preventDefault();addLoader(".enter-new-email");const form=$(this);resetErrors(form);const data=form.serialize()+`&recaptcha=${await getReCaptchaV3("change_email")}`;$.post("/change-email/send-code",data).done(()=>{form[0].reset();togglePopup("newEmail",false);openVerifyPopup("change_email","/change-email/verify-code","/change-email/resend-code",$(".current.email").text())}).fail(xhr=>{handleFail(form,xhr)}).always(()=>{removeLoader(".enter-new-email")})});$(document).on("change_email-verified",()=>{togglePopup("emailSuccesChanged",true)});$(".reset-btn.email").click(()=>{togglePopup("newEmail",true)});$("#pass-succes-changed .btn").click(()=>{togglePopup("passSuccesChanged",false)});$("#change-password-form").submit(async function(e){e.preventDefault();addLoader(".add-new-pass");const form=$(this);resetErrors(form);const data=form.serialize()+`&recaptcha=${await getReCaptchaV3("change_password")}`;$.post("/change-password",data).done(()=>{form[0].reset();togglePopup("passSuccesChanged",true)}).fail(xhr=>{handleFail(form,xhr)}).always(()=>{removeLoader(".add-new-pass")})});$(".reset-btn.pass").click(()=>{togglePopup("newPass",true)});$("#email-succes-changed .btn").click(()=>{togglePopup("emailSuccesChanged",false)});$(".open-faq").click(function(){$(".sidebar").toggleClass("open");$("html").addClass("lock")});$(document).ready(function(){$(".accordion-open").click(function(){$(this).closest(".accordion").toggleClass("active");var panel=$(this).next(".accordion-pannel");panel.slideToggle()});$(".accordion-item").click(function(){$(".accordion-item").removeClass("open");$(".sidebar-menu-item").removeClass("open");$(this).addClass("open");$(this).closest(".accordion").addClass("open")});$(".sidebar-menu-item").click(function(){if(!$(this).hasClass("accordion")){$(".accordion-item").removeClass("open");$(".sidebar-menu-item").removeClass("open");$(this).addClass("open")}});$("[data-target]").click(function(event){event.preventDefault();var targetId=$(this).data("target");$(".faq-content").hide();$("#"+targetId).show();updateMenuClasses(targetId)});$(".faq-next").click(function(){var current=$(".faq-content:visible");var next=current.next(".faq-content");if(next.length===0){next=$(".faq-content").first()}showContent(next.attr("id"));$(window).scrollTop(0)});$(".faq-prev").click(function(){var current=$(".faq-content:visible");var prev=current.prev(".faq-content");if(prev.length===0){prev=$(".faq-content").last()}showContent(prev.attr("id"));$(window).scrollTop(0)});function showContent(targetId){$(".faq-content").hide();$("#"+targetId).show();updateMenuClasses(targetId)}function updateMenuClasses(targetId){$(".accordion-item").removeClass("open");$(".sidebar-menu-item").removeClass("open");var targetLink=$('[data-target="'+targetId+'"]');targetLink.addClass("open");var accordion=targetLink.closest(".accordion");if(accordion.length){accordion.addClass("open");accordion.find(".accordion-pannel").slideDown()}else{$(".accordion").removeClass("active");$(".accordion-pannel").slideUp()}}});$(document).ready(function(){$(".search-input").on("input",function(){var query=$(this).val().toLowerCase().trim();var $searchResult=$(".search-result");$searchResult.empty();if(query){$(".faq-content").each(function(){var $this=$(this);var content=$this.text();var contentLower=content.toLowerCase();var id=$this.attr("id");var re=new RegExp(query,"gi");var match;var results=[];while((match=re.exec(contentLower))!==null){var start=Math.max(0,match.index-20);var end=Math.min(content.length,match.index+query.length+70);var snippet=content.substring(start,end);var highlightedSnippet=snippet.replace(new RegExp(query,"gi"),function(match){return"<span class='highlight'>"+match+"</span>"});results.push({snippet:highlightedSnippet,id:id})}results.forEach(function(result){$searchResult.append('<div class="result-item" data-target="'+result.id+'">'+result.snippet+"...</div>")})});$searchResult.show()}else{$searchResult.hide()}});$(document).on("click",function(event){if(!$(event.target).closest(".search-wrapper").length){$(".search-result").hide()}});$(".search-wrapper").on("click",function(event){event.stopPropagation()});$(".search-result").on("click",".result-item",function(){var targetId=$(this).data("target");$(".faq-content").hide();$("#"+targetId).show();$(".search-result").hide();$(".sidebar-menu-item, .accordion-item").removeClass("open").removeClass("active");var $targetSidebarItem=$(".sidebar-menu-item[data-target='"+targetId+"']");if($targetSidebarItem.length){$targetSidebarItem.addClass("open")}else{var $targetAccordionItem=$(".accordion-item[data-target='"+targetId+"']");if($targetAccordionItem.length){$targetAccordionItem.addClass("open");$targetAccordionItem.closest(".accordion-pannel").slideToggle();$targetAccordionItem.closest(".accordion").addClass("open");$targetAccordionItem.closest(".accordion").addClass("active")}}})});$(document).ready(function(){if($("header .advertising-banner").length){$(".open-faq").css("top","93px");$(".sidebar.open").css("top","93px")}else{$(".open-faq").css("top","60px");$(".sidebar.open").css("top","60px")}});$(document).on("click",".sidebar-menu-item",function(){$(".sidebar").removeClass("open");$("html").removeClass("lock")});$(".open-transaction").click(()=>{$(".transaction-wrapper").toggleClass("active")});$(".transaction-wrapper .close").click(()=>{$(".transaction-wrapper").toggleClass("active")});$(document).on("click",".subscribe-Btn:not(.after,.loading)",()=>{addLoader(".subscribe-Btn","6px");$(".subscribe-Btn").addClass("loading");$(".subscribe-card .text-error").removeClass("show");$.post("/subscribe").done(()=>{location.reload()}).fail(xhr=>{$(".subscribe-card .text-error").text(xhr.responseJSON.message).addClass("show")}).always(()=>{$(".subscribe-Btn").removeClass("loading");removeLoader(".subscribe-Btn")})});$(".un-subscribe-Btn").on("click",()=>{$(".unsubcribe-wrapper .text span").text($(".renewal span").text());$(".unsubcribe-wrapper").addClass("active")});$(".unsubcribe-wrapper .close").on("click",()=>{$(".unsubcribe-wrapper").removeClass("active")});$(".unsubcribe-wrapper .btn").on("click",()=>{$(".unsubcribe-wrapper").removeClass("active");addLoader(".un-subscribe-Btn","6px");$(".subscribe-card .text-error").removeClass("show");$.post("/unsubscribe").done(()=>{location.reload()}).fail(xhr=>{$(".subscribe-card .text-error").text(xhr.responseJSON.message).addClass("show")}).always(()=>{removeLoader(".un-subscribe-Btn")})});const deposit={payment_id:null};let depositQR=null;if(document.getElementById("deposit-qr")){depositQR=new QRCode(document.getElementById("deposit-qr"),"")}$(".deposit-type").on("click",async function(){deposit.payment_id=$(this).data("payment_id");addLoader(`[data-payment_id=${deposit.payment_id}]`,"4px");$.post("/payments/deposit",{gateway:"crypto",payment_id:deposit.payment_id,recaptcha:await getReCaptchaV3("deposit")}).done(res=>{$(".coins-wrapper").removeClass("active");$(".deposit-popup .network-icon").attr("src",$(this).find("img").attr("src"));$(".deposit-popup .crypto-name span").text($(this).data("network"));depositQR.clear();depositQR.makeCode(res.details.address_to);$(".network .type").text($(this).data("currency"));$(".deposit-wrapper .crypto-rate span").text(`${1/DS.rates[deposit.payment_id]} ${$(this).data("currency")}`);$(".key .title").text(res.details.address_to);$(".deposit-wrapper").addClass("active")}).always(()=>{removeLoader(`[data-payment_id=${deposit.payment_id}]`)})});$(".copy").on("click",()=>{let keyText=$(".key .title").text();let $tempInput=$("<input>");$("body").append($tempInput);$tempInput.val(keyText).select();document.execCommand("copy");$tempInput.remove();$(".message").show().delay(2e3).fadeOut()});$(".coins-wrapper .close").click(()=>{$(".coins-wrapper").toggleClass("active")});$(".deposit-wrapper .close").click(()=>{$(".deposit-wrapper").toggleClass("active")});const withdraw={payment_id:null,address_to:null,amount:null};$(".referral-out").on("click",function(){withdraw.payment_id=$(this).data("payment_id");$(".crypto-address .network-icon").attr("src",$(this).find("img").attr("src"));$(".crypto-address .crypto-name span").text($(this).data("network"));$(".crypto-address .crypto-rate span").text(`${1/DS.rates[withdraw.payment_id]} ${$(this).data("currency")}`);$(".referral-out-wrapper").addClass("active");$(".referral-out-wrapper .card").removeClass("active");$(".crypto-address").addClass("active")});$(".crypto-address .next").on("click",async()=>{withdraw.address_to=$("#cryptoAddress").val();withdraw.amount=parseInt($("#amount").val());$(".crypto-address-input").removeClass("error");$(".amount-input").removeClass("error");let valid=true;if(!withdraw.address_to){$(".crypto-address-input").addClass("error");$(".crypto-address-input").find(".error-text").text("Address is required");valid=false}if(!withdraw.amount){$(".amount-input").addClass("error");$(".amount-input").find(".error-text").text("Amount is required");valid=false}const max=parseInt($("#amount").attr("max"));if(withdraw.amount>max){$(".amount-input").addClass("error");$(".amount-input").find(".error-text").text("Not enough Coins");valid=false}if(!valid){return}addLoader(".crypto-address");$.post("/payments/withdraw/send-code",{...withdraw,gateway:"crypto",recaptcha:await getReCaptchaV3("withdraw")}).done(()=>{$(".referral-out-wrapper").removeClass("active");$(".crypto-address").removeClass("active");openVerifyPopup("withdraw","/payments/withdraw/verify-code","/payments/withdraw/resend-code",$(".user-mail").text())}).always(()=>{removeLoader(".crypto-address")})});$(document).on("withdraw-verified",()=>{$(".referral-out-wrapper").addClass("active");$(".withdrawn-final").addClass("active")});$(".withdrawn-final .btn").on("click",async()=>{$(".referral-out-wrapper").removeClass("active")});$(".referral-out-wrapper .back").on("click",function(){const card=$(this).closest(".card");card.removeClass("active");card.prev().addClass("active");if(card.hasClass("crypto-address")){$(".referral-out-wrapper").removeClass("active")}});$(".referral-out-wrapper .close").on("click",()=>{$(".referral-out-wrapper").removeClass("active")});$(".load-more img").on("click",()=>{$(".referal-item.none").each((i,item)=>{if(i>9){return}$(item).removeClass("none")});if(!$(".referal-item.none").length){$(".load-more").addClass("none")}});$(".copy-link").on("click",()=>{let linkText=$(".link-body .link").text();let $tempInput=$("<input>");$("body").append($tempInput);$tempInput.val(linkText).select();document.execCommand("copy");$tempInput.remove();$(".link-body .message").show().delay(2e3).fadeOut()});$(document).ready(function(){function togglePasteButton(){if($("#cryptoAddress").val()?.trim()!==""){$("#pasteButton").hide()}else{$("#pasteButton").show()}}togglePasteButton();$("#pasteButton").on("click",function(){navigator.clipboard.readText().then(text=>{$("#cryptoAddress").val(text);togglePasteButton()})});$("#cryptoAddress").on("input",function(){togglePasteButton()})});$(document).ready(function(){$("#description").on("input",function(){var descriptionLength=$(this).val().length;$(".counter").text(descriptionLength);if(descriptionLength<=250){$(".counter").css("color","red")}else{$(".counter").css("color","#dedfe1")}})});
=======
            </div>`;
        };
    ...
    $battle.find(".start_btn").click(function () {
        ...
        startProgressAnimation($circle, DS.ads.settings.repeat_time * 1e3);
        setTimeout(
            () => activateRepeatButton($this),
            DS.ads.settings.repeat_time * 1e3
        );
    });
    ...
    $battle.on("click", ".repeat.active", function () {
        ...
        restartProgressAnimation($circle, DS.ads.settings.repeat_time * 1e3);
        setTimeout(
            () => activateRepeatButton($this),
            DS.ads.settings.repeat_time * 1e3 + 1500
        );
    });
    function startProgressAnimation($circle, duration) {
        setTimeout(() => {
            $circle.css({
                transition: `stroke-dashoffset ${duration / 1e3}s linear`,
                strokeDashoffset: "0",
            });
        }, 0);
    }
    function restartProgressAnimation($circle, duration) {
        $circle.css({ transition: "none", strokeDashoffset: circumference });
        setTimeout(() => {
        ...
        if (query) {
            $(".faq-content").each(function () {
                ...
                while ((match = re.exec(contentLower)) !== null) {
                    ...
                    results.push({ snippet: highlightedSnippet, id: id });
                }
...
$(".copy").on("click", () => {
    ...
    $tempInput.remove();
    $(".message").show().delay(2e3).fadeOut();
});
...
const withdraw = { currency: null, to: null, amount: null };
...
$(".copy-link").on("click", () => {
    ...
    document.execCommand("copy");
    $tempInput.remove();
    $(".link-body .message").show().delay(2e3).fadeOut();
});
$(document).ready(function () {
